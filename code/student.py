...
def get_tiny_images(image_paths):
    ...
    # TODO: Implement this function!
    imgs_tiny = np.zeros((len(image_paths), 16*16))
    for i, img_path in enumerate(image_paths):
        # read the image
        img = cv2.imread(img_path)
        # convert to grayscale using skimage
        img_gray = rgb2gray(img)
        # resize the image to 16x16 using skimage
        img_tiny = resize(img_gray, (16, 16), anti_aliasing=True)
        # flatten the image
        img_tiny_flat = img_tiny.flatten()
        # add the flattened image to the output array
        imgs_tiny[i] = img_tiny_flat
    ...
    #     # convert to be 16X16
    #     img_read = cv2.resize(img_read, (16,16))
    #     # flatten
    #     img_read = img_read.reshape(256)
    #     imgs_tiny[i] = img_read


    # return imgs_tiny


def build_vocabulary(image_paths, vocab_size, cells_per_block=(2,2), pixels_per_cell=(8,8)):
    """
    ...
    for path in image_paths:
        ...
        # resize the image to 16x16 using skimage
        image = resize(image, (220, 293), anti_aliasing=True)


        # Compute the HOG descriptor for the image
        image_hog = hog(image, cells_per_block=cells_per_block, pixels_per_cell=pixels_per_cell, feature_vector=True)
        # Add the HOG descriptor to the features array
        if features is None:
            features = image_hog.reshape(1, -1)
        else:
            # print(features.shape, image_hog.shape,image_hog.reshape(1, -1).shape)
            try:
                features = np.vstack((features, image_hog.reshape(1, -1)))
                # print(image.shape)
            except:
                pass
                print("skipping unknown", image_hog.reshape(1, -1).shape)
                # print(features.shape)
                # print(image.shape, path)

    # Perform K-means clustering on the features
    kmeans = KMeans(n_clusters=vocab_size, max_iter=100, tol=0.1, verbose=1)
    kmeans.fit(features)
...
def get_bags_of_words(image_paths, cells_per_block=(2,2), pixels_per_cell=(8,8)):
    """
    ...
    # Set the dimensions of the histogram array
    n_images = len(image_paths)
    d = vocab.shape[0]
    histogram = np.zeros((n_images, d))
    ...
    for i, path in enumerate(image_paths):
        ...
        im = rgb2gray(im)
        # resize the image to 16x16 using skimage
        im = resize(im, (220, 293), anti_aliasing=True)



        # # Extract the features using HOG
        # features = hog(im, orientations=9, pixels_per_cell=(8, 8),
        #             cells_per_block=(2, 2), block_norm='L2-Hys')
        features = hog(im, cells_per_block=cells_per_block, pixels_per_cell=pixels_per_cell, feature_vector=True)

        # Calculate the distances to the nearest vocab words
        distances = cdist(features.reshape(1, -1), vocab)

        # Sort the distances to find the closest words
        closest_words = np.argsort(distances)

        # Build the histogram for this image
        for j in closest_words[0][:50]:
            histogram[i][j] += 1
